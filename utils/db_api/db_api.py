...
class SQLighter:
    ...
    def get_resume_all(self, allow=True):
        with self.connection:
            return self.cursor.execute("SELECT * FROM `tg_my_resume` WHERE `allow` = ? ORDER BY `date_time` DESC", (allow,)).fetchall()

    def update_resume_my(self, name, skills, area_of_residence, phone, user_id, allow=True):
        with self.connection:
            return self.cursor.execute("UPDATE `tg_my_resume` SET `name` = ? ,`skills` = ? ,`area_of_residence` = ?,`phone` = ? ,`allow` = ?, `date_time` = ?, `user_id_id` = ? WHERE `user_id_id` = ?", (name, skills,
